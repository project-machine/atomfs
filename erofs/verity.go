...
func appendVerityData(file string) (string, error) {
	...
	fmt.Printf("VERIFY1 - offset:%+v dev:%+v\n", verityOffset, verityDevice)
...
func verityDataLocation(sblock *superblock) (uint64, error) {
	return uint64((1 << sblock.BlockSizeBits) * sblock.Blocks), nil
}
...
func GuestMount(erofsFile string, mountpoint string) error {
	if isMountpoint(mountpoint) {
		return errors.Errorf("%s is already mounted", mountpoint)
	}

	abs, err := filepath.Abs(erofsFile)
	if err != nil {
		return errors.Errorf("Failed to get absolute path for %s: %v", erofsFile, err)
	}
	erofsFile = abs
	...
	cmd, err := erofsFuse(erofsFile, mountpoint)
	if err != nil {
		return err
	}
	if err := cmd.Process.Release(); err != nil {
		return errors.Errorf("Failed to release process after guestmount %s: %v", erofsFile, err)
	}
	return nil
}
